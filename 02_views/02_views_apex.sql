...
-- 8) KPIs para Cards no APEX
CREATE OR REPLACE VIEW v_kpi_vendas AS
SELECT
  SUM(
    CASE 
      WHEN v.status = 'FECHADA'
       AND TRUNC(v.dt_venda) = TRUNC(SYSDATE)
      THEN v.valor_liquido 
      ELSE 0 
    END
  ) AS receita_hoje,
  ...
  SUM(
    CASE 
      WHEN v.status = 'FECHADA'
       AND v.dt_venda >= TRUNC(SYSDATE) - 7
      THEN v.valor_liquido 
      ELSE 0 
    END
  ) AS receita_7d,
  ...
  SUM(
    CASE 
      WHEN v.status = 'FECHADA'
       AND v.dt_venda >= TRUNC(SYSDATE) - 30
      THEN v.valor_liquido 
      ELSE 0 
    END
  ) AS receita_30d,
  ...
  SUM(
    CASE 
      ...
      ELSE 0 
    END
  ) AS qtd_vendas_mes_atual
FROM tb_venda v;

-- 9) Série temporal: receita por dia nos últimos 30 dias
...
-- 10) Receita por categoria no mês atual
...
-- 11) Top 10 clientes no mês atual
...
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (ORDER BY SUM(v.valor_liquido) DESC) AS posicao
  FROM tb_venda v
  JOIN tb_cliente c 
    ON c.id_cliente = v.id_cliente
  WHERE v.status = 'FECHADA'
